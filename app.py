...
app.secret_key = os.environ.get("SESSION_SECRET",
                                "dev-secret-key-for-moel-legal")
app.wsgi_app = ProxyFix(app.wsgi_app, x_proto=1, x_host=1)

# Configure the database
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get(
    "DATABASE_URL", "postgresql://localhost/moel_legal")
...
def contact():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            app.logger.info(
                f"Contact form submitted successfully: {name}, {email}")

        except Exception as e:
            db.session.rollback()
            app.logger.error(f"Error processing contact form: {str(e)}")
            flash('حدث خطأ أثناء إرسال الرسالة. يرجى المحاولة مرة أخرى',
                  'error')
...
def send_admin_notification(contact_message):
    ...
    try:
        ...
        # Send email
        text = msg.as_string()
        server.sendmail(app.config['MAIL_DEFAULT_SENDER'],
                        app.config['ADMIN_EMAIL'], text)
        server.quit()

        app.logger.info(
            f"Admin notification email sent for contact from {contact_message.email}"
        )
...
def send_user_confirmation(contact_message):
    ...
    try:
        ...
        text = msg.as_string()
        server.sendmail(app.config['MAIL_DEFAULT_SENDER'],
                        contact_message.email, text)
        server.quit()

        app.logger.info(
            f"User confirmation email sent to {contact_message.email}")
